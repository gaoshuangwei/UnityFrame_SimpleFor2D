...
public static class LevelSystem
{
    ...
    public static List<Level> LoadLevels()
    {
        ...
        string filePath = Application.persistentDataPath + "/levels";//判断某个文件是否存在不需要扩展名
        FileInfo mydir = new FileInfo(filePath);
        if (!mydir.Exists)
        {
            xmlDoc.LoadXml(((TextAsset)Resources.Load("levels")).text);
            CreateXML(xmlDoc.InnerXml);
        }
        else
        {
           xmlDoc.Load(filePath+".xml"); //加载xml文件需要路径+文件名+扩展名
        }
    ...
    public static void CreateXML(string inner)
    {
        string path = Application.persistentDataPath + "/levels.xml";
        if (!File.Exists(path))
        {
            //通过string创建xml文件
